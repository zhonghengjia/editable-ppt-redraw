"""SVG paths from svg2pptx-skill (MIT); see LICENSE and THIRD_PARTY_NOTICES.

Local changes: strict input validation and closed-subpath current-point reset.
"""
...
def parse_svg_path(d: str) -> list[PathCommand]:
    ...
    matches = list(re.finditer(pattern, d))
    previous = 0
    for match in matches:
        if d[previous:match.start()].strip(' ,\t\r\n'):
            raise ValueError('Unsupported or malformed SVG path token')
        previous = match.end()
    if d[previous:].strip(' ,\t\r\n'):
        raise ValueError('Unsupported or malformed SVG path token')
    ...
    def flush() -> None:
        ...
        if n == 0 and current_args:
            raise ValueError('Closepath cannot have arguments')
        if n and (not current_args or len(current_args) % n):
            raise ValueError(f'Incomplete arguments for SVG path {current_cmd}')
        if n == 0:
    ...
    for token in tokens:
        if token in 'MmLlHhVvCcSsQqTtAaZz':
            ...
            if not math.isfinite(value) or abs(value) > 1e9:
                raise ValueError('SVG path coordinates must be finite and bounded')
            current_args.append(value)

    flush()
    for command in commands:
        if command.cmd.upper() == 'A':
            if command.args[3] not in (0, 1) or command.args[4] not in (0, 1):
                raise ValueError('Arc flags must be 0 or 1')
            if command.args[0] < 0 or command.args[1] < 0:
